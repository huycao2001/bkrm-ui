...
const CashierChangeCartBtn = (props) => {
    ...
    useEffect(()=>{
        console.log("con cac" + JSON.stringify(cartList))
    }, [cartList])
    const getTitle = (cart) => {
        if (isCart) {
    ...
    return (
        ...
        >
            {cartList.map((cart, index) => (
                <MenuItem
                    key={index}
                    selected={cart.uuid === selectedTakeAwayCart}
                >
                    <Grid container direction="row" justifyContent="space-between"spacing={1}>
                        <Grid container item onMouseDown={()=>handleChoose(cart.uuid)} xs={10}  >
                            <Box style={{marginRight:10}}>#{index + 1}</Box>
                            {getTitle(cart)}         
                        </Grid>
                
                        <Grid container xs={2} item>
                            <IconButton onClick={()=>handleDelete(cart.uuid )}  size="small" style={{color:'#000000'}}>
